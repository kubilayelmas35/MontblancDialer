...
// CAMPAIGNS â€” kampanya yÃ¶netimi
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
campaigns = campaigns || [];
renderCampGrid();
} catch(e){ toast('Kampanyalar yÃ¼klenemedi: '+e.message,'err'); console.error(e); }
}

function renderCampGrid() {
const grid = document.getElementById('camp-grid');
if (!campaigns.length) {
grid.innerHTML = `<div style="color:var(--text-3);padding:32px;text-align:center;grid-column:1/-1;font-size:13px;">HenÃ¼z kampanya yok</div>`;
return;
}
...
?'<span class="badge badge-green">â— Aktif</span>'
:c.status==='paused'
?'<span class="badge badge-yellow">â¸ DuraklatÄ±ldÄ±</span>'
:'<span class="badge badge-gray">âœ“ TamamlandÄ±</span>';
return `<div class="camp-card" onclick="openCampDetail('${c.id}')">
<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;">
<div><div class="camp-name">${c.name}</div><div class="camp-did">${c.telnyx_did||'DID atanmadÄ±'}</div></div>
<div style="display:flex;gap:4px;flex-shrink:0;" onclick="event.stopPropagation()">
${sb2}
<button class="btn-icon" onclick="openCampFieldSettings('${c.id}')" style="font-size:10px;padding:2px 6px;white-space:nowrap;">âš™ï¸</button>
<button class="btn btn-ghost btn-sm" onclick="deleteCampaign('${c.id}')" style="width:26px;height:26px;font-size:11px;">ğŸ—‘</button>
</div>
</div>
<div class="camp-kpi">
...
<span>Ä°lerleme</span><span style="color:var(--accent);font-weight:700;font-family:var(--mono);">${pct}%</span>
</div>
<div class="prog-wrap"><div class="prog-fill" style="width:${pct}%;"></div></div>
</div>
<div style="display:flex;align-items:center;justify-content:space-between;">
<span style="font-size:12px;color:var(--text-3);">HÄ±z: <strong style="color:var(--accent);font-family:var(--mono);">${c.dial_speed} hat</strong></span>
<button class="btn btn-ghost btn-xs" onclick="event.stopPropagation();toggleCampStatus('${c.id}','${c.status}')">
${c.status==='active'?'â¸ Duraklat':'â–¶ BaÅŸlat'}
</button>
</div>
</div>`;
}).join('');
...
document.getElementById('cd-did').textContent  = c.telnyx_did||'DID atanmadÄ±';
...
<button onclick="openRequeueModal('${q.id}','${q.name}')" title="KuyruÄŸu YÃ¶net" style="background:var(--bg-3);border:1px solid var(--border);border-radius:5px;padding:3px 8px;cursor:pointer;font-size:11px;color:var(--text-2);">âš™ YÃ¶net</button>
</div>`;}).join('')
: `<div style="color:var(--text-3);font-size:12px;text-align:center;padding:16px;">Kuyruk yok â€” yÃ¼kle!</div>`;
...
<button class="rm" onclick="removeAgent('${ac.agent_id}','${id}')">âœ•</button>
</div>`).join('')
: `<span style="color:var(--text-3);font-size:12px;">Agent atanmadÄ±</span>`;
...
ncFirmSel.innerHTML = '<option value="">Firma seÃ§in...</option>' +
...
<span style="color:var(--text-3);font-size:14px;cursor:grab;user-select:none;">â ¿</span>
<input type="checkbox" ${f.show?'checked':''} ${f.locked?'disabled':''}
onchange="_ncFields[${i}].show=this.checked;updateCampPreview()" title="GÃ¶ster/Gizle">
<input class="form-input" value="${f.label}" style="flex:1;font-size:12px;padding:3px 7px;"
oninput="_ncFields[${i}].label=this.value;updateCampPreview()" placeholder="Alan adÄ±">
...
<option value="number" ${f.type==='number'?'selected':''}>SayÄ±</option>
<option value="date" ${f.type==='date'?'selected':''}>Tarih</option>
<option value="boolean" ${f.type==='boolean'?'selected':''}>Evet/HayÄ±r</option>
<option value="select" ${f.type==='select'?'selected':''}>AÃ§Ä±lÄ±r MenÃ¼</option>
<option value="multiselect" ${f.type==='multiselect'?'selected':''}>Ã‡oktan SeÃ§meli</option>
<option value="checkbox" ${f.type==='checkbox'?'selected':''}>Checkbox</option>
</select>
${f.locked
? '<span style="font-size:10px;color:var(--text-3);">ğŸ”’</span>'
: `<button onclick="ncRemoveField(${i})" style="background:none;border:none;color:var(--red);cursor:pointer;font-size:14px;padding:0 2px;">âœ•</button>`
}
</div>`).join('');
}
...
label: 'Ã–zel Alan ' + (_ncFields.filter(f=>f.key.startsWith('custom')).length+1),
...
<span style="font-size:11px;color:var(--text-3);font-weight:700;">Åablon:</span>
<select class="form-input" id="nc-template-sel" style="font-size:11px;padding:3px 8px;max-width:150px;">
<option value="">SeÃ§...</option>
${names.map(n=>`<option value="${n}">${n}</option>`).join('')}
</select>
<button class="btn btn-ghost btn-sm" onclick="ncLoadTemplate()" style="font-size:11px;">YÃ¼kle</button>
...
renderNcFieldList();
updateCampPreview();
toast(`"${name}" ÅŸablonu yÃ¼klendi`, 'ok');
}

function ncDeleteTemplate() {
const sel = document.getElementById('nc-template-sel');
const name = sel?.value;
if (!name || !confirm(`"${name}" ÅŸablonu silinsin?`)) return;
const tpls = getFieldTemplates();
delete tpls[name];
saveFieldTemplates(tpls);
renderNcTemplateSelect();
toast('Åablon silindi', 'ok');
}
...
if (!visible.length) {
el.innerHTML = '<div style="color:var(--text-3);text-align:center;padding:20px;font-size:12px;">Alan seÃ§ilmedi</div>';
return;
}
const sampleVal = t => t==='number'?'123':t==='date'?'01.01.2024':t==='boolean'?'Evet':'Ã–rnek veri';
el.innerHTML = `
...
if (!name) { toast('Kampanya adÄ± gerekli','err'); return; }
const targetFirmId = (isSuperAdmin() ? document.getElementById('nc-firm-id')?.value : null) || getActiveFirmId() || currentUser?.firm_id;
if (!targetFirmId) { toast('Firma seÃ§in veya yeniden giriÅŸ yapÄ±n','err'); return; }
const fieldConfig = {};
_ncFields.forEach(f => {
...
toast('Kampanya oluÅŸturuldu âœ“','ok');
loadCampaigns();
...
if (!confirm('Bu kampanyayÄ± silmek istediÄŸine emin misin?')) return;
try {
...
toast(`HÄ±z ${v} olarak kaydedildi`,'ok');
} catch(e){}
}

// â”€â”€ Alan ayarlarÄ± â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function openCampFieldSettings(campId) {
const camp = campaigns.find(c => c.id === campId);
if (!camp) { toast('Kampanya bulunamadÄ±','err'); return; }
_fsCampId = campId;
let fc = {};
...
{key:'city',    defaultLabel:'Åehir',        type:'text',   show:true,  locked:false},
{key:'address', defaultLabel:'Adres',        type:'text',   show:true,  locked:false},
{key:'attempt', defaultLabel:'Arama SayÄ±sÄ±', type:'number', show:true,  locked:false},
{key:'notes',   defaultLabel:'Notlar',       type:'text',   show:false, locked:false},
{key:'custom1', defaultLabel:'Ã–zel Alan 1',  type:'text',   show:false, locked:false},
{key:'custom2', defaultLabel:'Ã–zel Alan 2',  type:'text',   show:false, locked:false},
{key:'custom3', defaultLabel:'Ã–zel Alan 3',  type:'number', show:false, locked:false},
];
...
<div class="modal-title">âš™ï¸ Alan AyarlarÄ± â€” ${camp.name}</div>
<button class="modal-close" onclick="document.getElementById('m-field-settings').remove()">âœ•</button>
</div>
...
<div style="font-size:11px;color:var(--text-3);margin-bottom:4px;">âœ“ GÃ¶ster/gizle Â· Ad ve tip deÄŸiÅŸtir</div>
...
<div style="font-size:10px;font-weight:800;color:var(--text-3);text-align:center;margin-bottom:10px;letter-spacing:.5px;">AGENT Ã‡AÄRI EKRANI Ã–NÄ°ZLEME</div>
...
<button class="btn btn-ghost" onclick="document.getElementById('m-field-settings').remove()">Ä°ptal</button>
<button class="btn btn-primary" onclick="saveCampFieldSettings('${campId}')">ğŸ’¾ Kaydet</button>
</div>
</div>`;
...
<div style="font-size:9px;color:var(--text-3);margin-bottom:3px;">SeÃ§enekler (virgÃ¼lle ayÄ±r):</div>
<input class="form-input" style="font-size:10px;padding:2px 5px;width:100%;"
placeholder="SeÃ§enek 1, SeÃ§enek 2"
...
<input class="form-input" value="${f.label||''}" style="flex:1;font-size:12px;padding:3px 7px;"
oninput="_fsFields[${i}].label=this.value;updateFsPreview()" placeholder="Alan adÄ±">
...
const sv = t=>t==='number'?'123':t==='date'?'01.01.2024':t==='boolean'?'Evet':'Ã–rnek';
el.innerHTML = `
<div style="font-size:12px;font-weight:800;padding:7px 10px;background:var(--accent);color:#fff;border-radius:6px;margin-bottom:8px;">ğŸ“ +49 176 1234567</div>
<div style="display:grid;grid-template-columns:1fr 1fr;gap:5px;">
...
document.getElementById('m-field-settings')?.remove();
toast('Alan ayarlarÄ± kaydedildi âœ“','ok');
loadCampaigns();
} catch(e) { toast('Hata: '+e.message,'err'); }
}

// â”€â”€ Kampanya Detay Sekmeleri â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const DEFAULT_TF = [
  ...
  {key:'baujahr',label:'YapÄ±m YÄ±lÄ±',type:'text',enabled:true,required:true},
  {key:'qm',label:'mÂ²',type:'number',enabled:true,required:true},
  {key:'heizung',label:'IsÄ±tma',type:'select',opts:'Gas,Ã–l,Pellet,WP,FernwÃ¤rme',enabled:true,required:true},
  {key:'alter_der_heizung',label:'IsÄ±tma YaÅŸÄ±',type:'text',enabled:true,required:true},
  {key:'verbrauch_pro_jahr',label:'TÃ¼ketim/YÄ±l',type:'text',enabled:true,required:false},
  {key:'personen',label:'KiÅŸi SayÄ±sÄ±',type:'number',enabled:true,required:false},
  {key:'interesse_an_pv',label:'PV Ä°lgisi',type:'boolean',enabled:false,required:false},
];
...
${['select'].includes(f.type)?`<div style="padding:3px 10px 6px;"><input class="form-input" id="tf-opts-${i}" style="font-size:10px;padding:2px 6px;width:100%;" placeholder="SeÃ§enekler (virgÃ¼lle)" value="${f.opts||''}"></div>`:''}
`).join('');
...
try {
  ...
  toast('Termin form ayarlarÄ± kaydedildi âœ“','ok');
  loadCampaigns();
} catch(e) { toast('Hata: '+e.message,'err'); }
}
...
<button class="btn btn-ghost btn-sm" style="margin-top:8px;" onclick="addFsField()">+ Ã–zel Alan Ekle</button>
<button class="btn btn-primary btn-sm" style="margin-top:8px;margin-left:4px;" onclick="saveCampFieldSettings('${currentCampId}')">ğŸ’¾ Kaydet</button>
</div>
<div>
<div style="font-size:12px;font-weight:700;margin-bottom:8px;text-align:center;color:var(--text-3);">Ã–NIZLEME</div>
...
${f.locked?'<span style="font-size:9px;color:var(--text-3);">Kilitli</span>':`<button onclick="_fsFields.splice(${i},1);renderCdFsFieldList();updateCdFsPreview()" style="background:none;border:none;color:var(--red);cursor:pointer;font-size:12px;">âœ•</button>`}
</div>${optHtml}
</div>`;
}).join('');
}
...
const vis = _fsFields.filter(f=>f.show);
const sv = t=>t==='number'?'123':t==='date'?'01.01.2024':t==='boolean'?'Evet':'Ã–rnek';
el.innerHTML = `<div style="font-size:11px;font-weight:800;padding:5px 8px;background:var(--accent);color:#fff;border-radius:5px;margin-bottom:6px;">ğŸ“ +49 176 1234567</div>
...
toast('Arama ayarlarÄ± kaydedildi âœ“', 'ok');
}
...
<button onclick="removeAuxCode(${i})" style="background:none;border:none;color:var(--red);cursor:pointer;">âœ•</button>
</div>`).join('');
}
...
div.innerHTML=`<input class="form-input" value="Yeni Kod" id="aux-code-${idx}" style="flex:1;font-size:12px;padding:4px 8px;"><button onclick="this.parentElement.remove()" style="background:none;border:none;color:var(--red);cursor:pointer;">âœ•</button>`;
el.appendChild(div);
}
...
const catLabels = {appointment:'âœ… BaÅŸarÄ±lÄ±', negative:'âŒ Negatif', callback:'ğŸ”„ Geri Ara', neutral:'âšª NÃ¶tr'};
...
${o.key && ['appointment','negative','callback','no_answer','dnc','voicemail'].includes(o.key)
? ''
: `<button onclick="removeCustomOutcome(${i})" style="background:none;border:none;color:var(--red);cursor:pointer;font-size:12px;">âœ•</button>`
}
</div>`).join('') +
`<button class="btn btn-ghost btn-sm" style="margin-top:6px;" onclick="addCustomOutcome()">+ SonuÃ§ Ekle</button>`;
}
...
<input class="form-input" value="Yeni SonuÃ§" id="outcome-label-${idx}" style="flex:1;font-size:12px;padding:3px 7px;">
<select class="form-input" id="outcome-cat-${idx}" style="font-size:11px;padding:2px 4px;width:110px;">
<option value="appointment">âœ… BaÅŸarÄ±lÄ±</option>
<option value="negative">âŒ Negatif</option>
<option value="callback">ğŸ”„ Geri Ara</option>
<option value="neutral" selected>âšª NÃ¶tr</option>
</select>
<button onclick="this.closest('div').remove()" style="background:none;border:none;color:var(--red);cursor:pointer;font-size:12px;">âœ•</button>`;
...
await saveCampSettingsPatch(s);
toast('QC ayarlarÄ± kaydedildi âœ“', 'ok');
}
...
toast('Kayan yazÄ± yayÄ±nlandÄ± âœ“', 'ok');
}

async function activateCampaignForAgents() {
const msg = prompt('Agentlere gÃ¶nderilecek mesaj (opsiyonel):','');
...
toast('Kampanya aktifleÅŸtirildi âœ“','ok');
}
...
// ── Kuyruk Yönetimi (Tekrar Ara) ─────────────────

async function openRequeueModal(queueId, queueName) {
  document.getElementById('m-requeue')?.remove();
  let counts = {};
  try {
    const rows = await sb(`contacts?queue_id=eq.${queueId}&select=status`);
    (rows||[]).forEach(r => { counts[r.status] = (counts[r.status]||0)+1; });
  } catch(e) {}

  const statuses = [
    { key:'no_answer', label:'Cevap Yok',  color:'var(--text-2)' },
    { key:'negative',  label:'Olumsuz',     color:'var(--red)'    },
    { key:'callback',  label:'Geri Ara',    color:'var(--yellow)' },
    { key:'calling',   label:'Aranıyor',    color:'var(--accent)' },
    { key:'dnc',       label:'Kara Liste',  color:'var(--red)'    },
  ];

  const m = document.createElement('div');
  m.className = 'modal-overlay'; m.id = 'm-requeue';
  m.innerHTML = `<div class="modal" style="max-width:440px;width:94%;">
<div class="modal-header">
  <div class="modal-title">⚙ Kuyruk Yönetimi — ${queueName}</div>
  <button class="modal-close" onclick="document.getElementById('m-requeue').remove()">✕</button>
</div>
<div style="padding:16px 20px;display:flex;flex-direction:column;gap:12px;">
  <div style="font-size:12px;color:var(--text-3);">Seçili statüdeki kişileri tekrar <b>beklemede (pending)</b> yap.</div>
  <div style="display:flex;flex-direction:column;gap:6px;">
    ${statuses.map(s => {
      const cnt = counts[s.key]||0;
      return `<label style="display:flex;align-items:center;gap:10px;padding:8px 10px;background:var(--bg-3);border-radius:6px;cursor:${cnt?'pointer':'default'};opacity:${cnt?'1':'0.4'};">
<input type="checkbox" class="rq-chk" value="${s.key}" ${cnt?'':'disabled'} style="width:15px;height:15px;">
<span style="flex:1;font-size:12px;font-weight:600;color:${s.color};">${s.label}</span>
<span style="font-size:11px;color:var(--text-3);background:var(--bg-2);border:1px solid var(--border);border-radius:10px;padding:2px 8px;">${cnt.toLocaleString()} kişi</span>
</label>`;
    }).join('')}
  </div>
  <div style="padding:8px 10px;background:var(--bg-3);border-radius:6px;">
    <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
      <input type="checkbox" id="requeue-reset-attempts" style="width:15px;height:15px;">
      <span style="font-size:12px;">Arama sayaçlarını sıfırla (attempt_count = 0)</span>
    </label>
  </div>
  <div style="font-size:12px;font-weight:700;color:var(--accent);min-height:18px;" id="requeue-preview"></div>
</div>
<div class="modal-footer">
  <button class="btn btn-ghost" onclick="document.getElementById('m-requeue').remove()">İptal</button>
  <button class="btn btn-primary" id="requeue-confirm-btn" onclick="doRequeue('${queueId}')">Tekrar Kuyruğa Al</button>
</div>
</div>`;
  document.body.appendChild(m);

  const updatePreview = () => {
    const sel = [...m.querySelectorAll('.rq-chk:checked')].map(c=>c.value);
    const total = sel.reduce((s,k)=>s+(counts[k]||0),0);
    document.getElementById('requeue-preview').textContent =
      total ? `→ ${total.toLocaleString()} kişi pending e alınacak` : '';
  };
  m.querySelectorAll('.rq-chk').forEach(cb => cb.addEventListener('change', updatePreview));
}

async function doRequeue(queueId) {
  const m = document.getElementById('m-requeue');
  if (!m) return;
  const checked = [...m.querySelectorAll('.rq-chk:checked')].map(c=>c.value);
  if (!checked.length) { toast('En az bir statu secin','err'); return; }
  const resetAttempts = m.querySelector('#requeue-reset-attempts')?.checked;

  const btn = document.getElementById('requeue-confirm-btn');
  if (btn) { btn.disabled = true; btn.textContent = 'Isleniyor...'; }

  try {
    const patch = { status:'pending', callback_at: null };
    ...
    const dialedRows = await sb(`contacts?queue_id=eq.${queueId}&status=not.in.(pending,calling)&select=id`);
    await sb(`queues?id=eq.${queueId}`, {
      method:'PATCH', prefer:'return=minimal',
      body: JSON.stringify({ dialed_count: (dialedRows||[]).length })
    });
    toast('Kisiler tekrar kuyruga alindi','ok');
    m.remove();
    if (currentCampId) setTimeout(()=>openCampDetail(currentCampId), 300);
  } catch(e) {
    toast('Hata: '+e.message,'err');
    if (btn) { btn.disabled = false; btn.textContent = 'Tekrar Kuyruga Al'; }
  }
}
